...
import { AddQuestionToTestCommand } from './add-question-to-test.command';
import {
...
@CommandHandler(AddQuestionToTestCommand)
export class AddQuestionToTestCommandHandler implements ICommandHandler<
...
> {
  ...
  async execute(command: AddQuestionToTestCommand): Promise<void> {
    ...
    await this.repository.addQuestion(command.testId, command.questionId);
  }
}
